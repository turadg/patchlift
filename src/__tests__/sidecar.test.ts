...
describe("sidecar", () => {
  ...
  it("sidecarPath stores sidecars in a sibling .patchlift directory", async () => {
    ...
    expect(sidecarPath(".yarn/patches/foo.patch")).toBe(".yarn/patches/.patchlift/foo.patch.yml");
    expect(sidecarPath("patches/foo.patch")).toBe("patches/.patchlift/foo.patch.yml");
    expect(sidecarPath("foo.patch")).toBe(".patchlift/foo.patch.yml");
  });
  ...
  it("writeSidecar creates the .patchlift directory", async () => {
    ...
    expect(sidecarPath(TEST_PATCH)).toBe(".patchlift/test-patch-sidecar-test.patch.yml");
  });
